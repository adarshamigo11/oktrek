...
/**
 * Fixed-window rate limit backed by MongoDB rate_limit_buckets.
...
export function rateLimit({ name, limit, windowSec, key }) {
  return async (req, res, next) => {
    try {
      ...
      const row = await db.collection("rate_limit_buckets").findOne({ _id: bucketKey });

      if (!row || new Date(row.window_start) < windowStartCutoff) {
        await db.collection("rate_limit_buckets").updateOne(
          { _id: bucketKey },
...
/** Default API limiter: 120/min/IP. */
